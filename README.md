# Flex Tokens Liftoff: EASY, GRAMS, WON & MEME

Get ur EASY, choose your reward, start stacking.

**Notes**  
Claim your badge by badgering admins in [t.me/flextokens](https://t.me/flextokens)

## Onboard to EASY

- Get a [**Wallet**](https://webauth.com) and fund it with XPR or USD(c) via CEX like [KuCoin](https://www.kucoin.com/), or use [crypto bridge](https://www.flex.town/) / [card](https://webauth.com) after KYC
- [Swap](https://proton.alcor.exchange/swap?input=XUSDC-xtokens&output=EASY-mon3y) to buy **[EASY](https://proton.alcor.exchange/swap?input=XUSDC-xtokens&output=EASY-mon3y)** here or choose **[WON](https://proton.alcor.exchange/swap?output=WON-w3won)**, **[MEME](https://proton.alcor.exchange/swap?output=MEME-m3m3)**, or **[GRAMS](https://proton.alcor.exchange/swap?output=GRAMS-gold.mon3y)**
- Flex: (change) your reward token, send out rewards + set a beneficiary via [flex.town](https://flex.town) — and [request invite](https://flex.town) to the **Welcome program**
- Take it EASY — Stack rewards + tell friends about EASY

Bookmark **[flex.town](https://flex.town)** to send reflections, change rewards, and more.
